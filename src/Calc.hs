    ...
    processRow (TransactionBuySell row) state = case getAction row of
      ...
      Sell ->
        let prevQuanitity = currQuantity state
         ...
         in newState
    -- dividend
    processRow (TransactionDividend row) state =
      state
  ...
  where
    avgBuy =
      if amount == 0 && quantity == 0
        then -- there are sutiations where this makes sense. E.g. If a company has
        -- changed name
          0
