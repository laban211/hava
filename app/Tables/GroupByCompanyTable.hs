...
import           Types.PrintableCell            ( FixedWidth(FixedWidth, l, s)
                                                , PrintableCell
                                                    ( textAlign
                                                    , width
                                                    )
...
createGroupByCompanyTable uiSize termWidth rows = createPrettyTable
    uiSize
    termWidth
    header
    content
...
groupByCompanyContent rows header =
    let company =
            getCompany . extractGenericTransaction . head $ getSortedByDateList
                rows
